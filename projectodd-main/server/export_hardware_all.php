...
header('Content-Disposition: attachment;filename="hardware_all_' . date('Ymd') . '.xls"');
header('Cache-Control: max-age=0');

$result = mysqli_query($koneksi, "SELECT * FROM hardware ORDER BY lokasi, nama_perangkat ASC");

echo "All Hardware Data - BPJS Ketenagakerjaan\n";
echo "Tanggal Export: " . date('d/m/Y H:i:s') . "\n\n";

echo "Lokasi\tType\tNama\tIP Console\tTahun\tSerial Number\tRack\tOS\tNo Inventaris\tFungsi\tStatus\tKeterangan\tEnd of Support\tVendor Support\tKontrak Start\tKontrak Finish\tBAST\tPartner\n";
