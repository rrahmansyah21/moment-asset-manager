...
<head>
    ...
    <title>Moment-Hardware DRC</title>
    ...
    <style>
        ...
        /* Style Tambahan untuk Kolom Aksi */
        .action-column {
            width: 90px;
            text-align: center;
            white-space: nowrap;
        ...
        /* Gaya untuk link sidebar yang aktif */
...
<body class="sb-nav-fixed">
            ...
            <nav class="sb-sidenav accordion sb-sidenav-dark" id="sidenavAccordion">
                <div class="sb-sidenav-menu">
                    <div class="nav">
                        ...
                        <div class="collapse show" id="collapseHardware" aria-labelledby="headingOne"
                            data-bs-parent="#sidenavAccordion">
                            <nav class="sb-sidenav-menu-nested nav">
                                <a class="nav-link" href="hardware_dc.php">DC</a>
                                <a class="nav-link active" href="hardware_drc.php">DRC</a>
                            </nav>
                        </div>
                    ...
                    <div class="card mb-4">
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between mb-2">
                                ...
                                <div>
                                    <a href="export_hardware_drc.php" class="btn btn-success me-2">
                            ...
                            <table id="datatablesSimple" class="table table-striped table-bordered table-sm table-compact mb-0">
                                ...
                                <tbody>
                                    <?php
                                    $result = mysqli_query($koneksi, "SELECT * FROM hardware WHERE lokasi = 'DRC' ORDER BY nama_perangkat ASC");
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        if ($is_admin) {
                                            ...
                                            $d_lokasi = htmlspecialchars($row['lokasi'] ?? 'DRC', ENT_QUOTES);
    ...
    <script>
    // Ensure the datatable for DRC is initialized and searchable.
    document.addEventListener('DOMContentLoaded', function () {
        try {
            const tbl = document.getElementById('datatablesSimple');
            if (!tbl) return;

            // If a previous instance exists, try to destroy it first
            if (window._drcTable && typeof window._drcTable.destroy === 'function') {
                try { window._drcTable.destroy(); } catch (e) { /* ignore */ }
            }

            // Create a fresh instance with searchable enabled
            window._drcTable = new simpleDatatables.DataTable(tbl, {
                searchable: true,
        ...
        // datatable initialized
            ...
            // If there's an external search input on the page (placeholder 'Search...'), wire it
            const extSearch = document.querySelector('input[placeholder="Search..."]');
            if (extSearch) {
                extSearch.addEventListener('input', function (e) {
                    if (window._drcTable) window._drcTable.search(e.target.value);
                });
                // external search input wired
            }
            } catch (err) {
                // datatable init error suppressed (logged during development)
            }
    });
    </script>
    <script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (!tableBody) {
            // table body not found (nothing to wire)
            return;
        }
        ...
        tableBody.addEventListener('click', function (e) {
            ...
            // attributes we expect (matches data-... attributes emitted by PHP)
            const attrs = [
                'id','type_perangkat','nama_perangkat','ip_console','tahun_pengadaan','serial_number',
                'rack','u_position','u_height','operating_system','no_iventaris','fungsi','status',
                'keterangan','end_of_support','support_vendor','kontrak_support_start','kontrak_support_finish',
                'tanggal_bast','lokasi','backup_type','ups_status','partner'
            ];

            // set id fields first (hidden + display)
            const dataId = btn.getAttribute('data-id');
            const hid = document.getElementById('edit_id');
            if (hid) hid.value = dataId || '';
            const disp = document.getElementById('display_id');
            if (disp) disp.value = dataId || '';

            // helper to set element value/text
            ...
            // populate all other fields by reading data- attributes directly
            attrs.forEach(function (attr) {
                if (attr === 'id') return; // already handled
                const dataVal = btn.getAttribute('data-' + attr);
                const el = document.getElementById('edit_' + attr);
                if (!el) {
                    // missing element: edit_ + attr (field may not exist in modal)
                }
                setElementValue(el, dataVal);
            });
        });
    });
    </script>
    <script>
    // Exposed helper so onclick="populateEditModal(this)" works even if event delegation misses
    function populateEditModal(btn) {
        if (!btn) return;
        // populateEditModal called
        // same attrs list as above
        ...
        attrs.forEach(function (attr) {
            ...
            const el = document.getElementById('edit_' + attr);
            if (!el) return;
            if (el.tagName.toLowerCase() === 'textarea' || el.tagName.toLowerCase() === 'input' || el.tagName.toLowerCase() === 'select') {
                el.value = dataVal !== null ? dataVal : '';
            } else {
                el.textContent = dataVal !== null ? dataVal : '';
            }
        });
    }
    </script>
